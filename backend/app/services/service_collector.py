...
Алгоритм (рекурсивный обход дерева категорий):
  ...
  5. Проставляем exclude_from_analytics из EXCLUDED_CATEGORY_IDS
...
async def _upsert_service(db: AsyncSession, item: dict) -> None:
    ...
    exclude = cat_id in EXCLUDED_CATEGORY_IDS if cat_id else False
    vals = dict(
        title                 = item["title"],
        category_id           = cat_id,
...
async def _traverse(
    ...
    if items:
        excluded = current.get("id") in EXCLUDED_CATEGORY_IDS
        flag = " [EXCLUDED]" if excluded else ""
        logger.info(f"{indent}[{category_id}] {current.get('title', '')}{flag} — {len(items)} услуг")
...
async def collect_services(db: AsyncSession) -> dict:
    ...
    Возвращает: {"categories": N, "services": N, "excluded": N}
    """
    client = await login()
    stats = {"categories": 0, "services": 0, "excluded": 0}
    ...
    try:
        ...
        excluded_count = await db.scalar(
            ...
            .where(Service.exclude_from_analytics == True)
        )
        stats["excluded"] = excluded_count or 0

        logger.info(
            f"[services] Готово: {stats['categories']} категорий, "
            f"{stats['services']} услуг ({stats['excluded']} исключено из аналитики)"
        )
    finally:
        await client.aclose()
